...
switch($_GET[act]){
  ...
  default:
	...
	if ($_SESSION[leveluser] == 'superadmin'){
		echo "<input type=button value='Tambah Pengguna' onclick=\"window.location.href='?modul=pengguna&act=tambah';\">";
    }      
    echo "<table>
          <tr><th>username</th><th>password</th><th>level</th><th>aksi</th></tr>"; 
	// Paging
  	$hal = $_GET[hal];
	if(!isset($_GET['hal'])){ 
		$page = 1; 
	...
	if ($_SESSION[leveluser] == 'superadmin'){
		...
		$tampil=mysql_query("SELECT * FROM pengguna WHERE username='$_SESSION[namauser]' ORDER BY username ASC LIMIT $offset, $jmlperhalaman");
	}
...
<?	   
  ...
  case "edit":
	...
	if ($_SESSION[leveluser] == 'superadmin'){
		...
		else  if ($r[level]=='superadmin'){
		  echo "<tr><td>Level</td>     <td> :  <input type=radio name='level' value='admin'> Admin
											   <input type=radio name='level' value='superadmin' checked> Superadmin
											   </td></tr>";
		}
	}else if ($_SESSION[leveluser] == 'admin'){
		if ($r[level]=='admin'){
